...
func init() {
	android.RegisterModuleType("FloydFlivercod_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = FloydFlivercodExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = FloydFlivercodExpandVariables(ctx, dep_file)
	...
	cmd := FloydFlivercodExpandVariables(ctx, String(g.properties.Cmd))
